...
(function () {
  ...
  const cmd2 = cp.spawn(
    `aws ssm get-parameters-by-path --with-decryption --path /hyfn/frontend/${env}/admin-app/BASE_URL`,
    { shell: true }
  );
  //   const cmd = cp.spawn(
  //     `aws ssm get-parameters-by-path --with-decryption --path /hyfn/admin-app/${buildInfo.envName}`,
  //     { shell: true },
  //   )
...
const after = async ({ cp, env, fs }) => {
  const cmd2 = cp.spawn(
    `aws ssm get-parameters-by-path --with-decryption --path /hyfn/frontend/${env}/admin-app`,
    { shell: true }
  );
