...
const validateLocalCardTransaction = async ({ arg, client }: MainFunctionProps) => {
  ...
  const response = await withTransaction({
    ...
    fn: async () => {
      ...
      if (transaction.type === TRANSACTION_TYPE_WALLET) {
        await client
          .db('generalData')
      ...
      if (transaction.type === TRANSACTION_TYPE_SUBSCRIPTION) {
