...
(function () {
  ...
  //   const cmd = cp.spawn(
  //     `aws ssm get-parameters-by-path --with-decryption --path /hyfn/products-library/${buildInfo.envName}`,
  //     { shell: true },
  //   )
...
const after = async ({ cp, env, fs }) => {
  const cmd2 = cp.spawn(
    `aws ssm get-parameters-by-path --with-decryption --path /hyfn/frontend/${env}/products-library`,
    { shell: true }
  );
