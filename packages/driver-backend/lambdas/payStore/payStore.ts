export const payStoreHandler = async ({ arg, session, client, event }: MainFunctionProps) => {
    var result;
    ...
    const productNotPicked = storeOrder.addedProducts.find((product) => product?.pickup?.pickedUp === undefined);
    if (productNotPicked) {
...
function calculateAmountToPayTheStoreAndAmountToReturnTheCustomer(storeOrder) {
    ...
    const amountToPay = storeOrder.addedProducts.reduce((accu, product) => {
        ...
        const amountToPayServiceFee = product.pricing.price * product?.pickup.QTYFound * storeAndCustomerServiceFee;
