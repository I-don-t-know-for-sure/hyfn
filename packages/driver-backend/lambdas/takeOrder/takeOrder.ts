export const takeOrderHandler = async ({ arg, session, client }: MainFunctionProps) => {
    ...
    const proposal = orderDoc.proposals.find((proposal) => proposal.managementId === orderDoc.acceptedProposal);
    if (!proposal) {
...
import { orderSchema, STORE_STATUS_PENDING, USER_TYPE_CUSTOMER, USER_TYPE_DRIVER, deliveryServiceFee, } from 'hyfn-types';
