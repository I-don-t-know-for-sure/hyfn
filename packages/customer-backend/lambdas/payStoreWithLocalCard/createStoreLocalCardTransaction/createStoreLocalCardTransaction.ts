...
import { STORE_STATUS_ACCEPTED, STORE_STATUS_PENDING, STORE_TYPE_RESTAURANT, paymentMethods, storeSchema, STORE_STATUS_PAID, } from 'hyfn-types';
...
const createStoreLocalCardTransaction = async ({ arg, client, session, }: createManagementLocalCardTransactionProps) => {
    ...
    const { amountToPay: amountToPayForProducts, amountToReturnToCustomer, originalCost, } = calculateAmountToPayTheStoreAndAmountToReturnTheCustomer({
        storeOrder,
    });
    const amountToPay = add(amountToPayForProducts, 0);
    console.log('🚀 ~ file: createStoreLocalCardTransaction.ts:127 ~ createStoreLocalCardTransaction ~ amountToPayForProducts:', amountToPayForProducts);
    ...
    //       storeOrder,
    //     });
    console.log('🚀 ~ file: createStoreLocalCardTransaction.js ~ line 36 ~ createStoreLocalCardTransaction ~ amountToPay', amountToPay);
    const { TerminalId, MerchantId, secretKey: encryptedSecretKey } = storeDoc.localCardAPIKey;
    console.log('🚀 ~ file: createStoreLocalCardTransaction.js ~ line 48 ~ createStoreLocalCardTransaction ~ encryptedSecretKey', encryptedSecretKey);
    console.log('🚀 ~ file: createStoreLocalCardTransaction.js ~ line 48 ~ createStoreLocalCardTransaction ~ encryptedSecretKey', encryptedSecretKey);
    const kmsKeyARN = process.env.kmsKeyARN || '';
    const secretKey = await decryptData({ data: encryptedSecretKey, kmsKeyARN, kmsClient });
    console.log('🚀 ~ file: createStoreLocalCardTransaction.js ~ line 53 ~ createStoreLocalCardTransaction ~ secretKey', secretKey);
