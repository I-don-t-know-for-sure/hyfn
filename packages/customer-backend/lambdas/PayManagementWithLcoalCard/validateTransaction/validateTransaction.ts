...
import { MainFunctionProps, decryptData, isLocalCardTransactionValidated, mainWrapper, mainWrapperWithSession, } from 'hyfn-server';
...
export const validateManagmentLocalCardTransaction = async ({ arg, client, session, }: ValidateManagmentLocalCardTransactionPropos) => {
    ...
    await client
        .db('generalData')
        .collection('driverManagement')
        .updateOne({ _id: new ObjectId(transactionDoc.storeId) }, {
        $inc: {
    ...
    await client
        ...
        .updateOne({ _id: new ObjectId(transactionDoc.customerId) }, {
        $set: {
    ...
    await client
        .db('base')
        .collection('orders')
        .updateOne({ _id: new ObjectId(transactionDoc.orderId) }, {
        $set: {
