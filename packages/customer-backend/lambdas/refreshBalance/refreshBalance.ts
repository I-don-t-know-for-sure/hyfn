export const refreshBalanceHandler = async ({ arg, client }: MainFunctionProps) => {
  const { userId } = arg[0];
...
interface RefreshBalanceProps extends Omit<MainFunctionProps, 'arg'> {}
import { mainWrapper } from 'hyfn-server/src';
import { MainFunctionProps } from 'hyfn-server/src';
export const handler = async (event) => {
  return await mainWrapper({ event, mainFunction: refreshBalanceHandler });
};
