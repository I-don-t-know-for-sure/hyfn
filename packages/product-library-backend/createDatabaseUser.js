...
(async function () {
  ...
  console.log(
    "🚀 ~ file: createDatabaseUser.js:27 ~ awsResult ~ awsResult",
    awsResult.stdout
  );
  const lambdaRoleName = JSON.parse(awsResult?.stdout)[0].PhysicalResourceId;
  console.log(
    "🚀 ~ file: createDatabaseUser.js:40 ~ lambdaRoleName",
    lambdaRoleName
  );
  const awsArn = await new Promise(function (resolve, reject) {
    exec(
      `aws iam get-role --role-name ${lambdaRoleName}`,
      (err, stdout, stderr) => {
        if (err) {
          reject(err);
        } else {
  ...
  const awsSSMResult = await new Promise(function (resolve, reject) {
    exec(
      ...
      --name "/hyfn/frontend/${env}/products-library/BASE_URL" \
      --value "${gatewayURL}" \
      --type "SecureString" \
      --overwrite`,
      (err, stdout, stderr) => {
  ...
  console.log(
    "🚀 ~ file: createDatabaseUser.js:44 ~ awsResult ~ awsResult",
    awsSSMResult
  );
})();
