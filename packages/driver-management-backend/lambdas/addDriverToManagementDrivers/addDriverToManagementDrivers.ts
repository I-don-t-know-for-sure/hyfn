export const addDriverToManagementDriversHandler = async ({ arg, client, userId, session, }: MainFunctionProps) => {
    ...
    console.log('🚀 ~ file: addDriverToStoreDrivers.js:14 ~ mainFunction ~ trustedBalance', trustedBalance);
    const { _id, usedBalance: managementUsedBalance, balance: managementBalance, verified, } = userDocument;
    const managementUsedBalanceFloat = parseFloat(managementUsedBalance.toFixed(3));
    console.log('🚀 ~ file: addDriverToManagementDrivers.js:20 ~ mainFunction ~ managementUsedBalanceInt', managementUsedBalanceFloat);
