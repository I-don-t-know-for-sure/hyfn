...
import { isLocalCardTransactionValidated, MainFunctionProps, mainWrapperWithSession, } from 'hyfn-server';
...
const validateLocalCardTransaction = async ({ arg, client, session }: MainFunctionProps) => {
    ...
    console.log('🚀 ~ file: validateLocalCardTransaction.js ~ line 12 ~ validateLocalCardTransaction ~ transactionId', transactionId);
    ...
    console.log('herehh');
    if (isValidated) {
        await client.db('generalData').collection('transactions').updateOne({ _id: transaction._id }, {
            validated: true,
        });
        await client
            .db('generalData')
