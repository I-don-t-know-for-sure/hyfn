import { Box, Button, CardSection, Container, Input, Radio, Select, Text, TextInput } from '@mantine/core'
...
const CollectionConditions: React.FC<CollectionconditionsProps> = ({
  ...
  const inputType = (condition: Condition) => {
    if (condition.objectKey === 'weight' || condition.objectKey === 'pricing.price') return 'number'
      ...
      <CardSection>
        <Box mt={'md'}>
          ...
          >
            ...
            />
            <Radio value="$or" label={t('Any Condition')} checked={collectionInfo.conditions?.mustMatch === '$or'} />
          </Radio.Group>
        </Box>
        <Box mt={'md'}>
            ...
            <Box key={condition.key}>
              <Box>
                ...
                >
                  <Select
                    ...
                    onChange={(e: Condition['objectKey']) => {
                      const updatedConditions = collectionInfo.conditions.conditionArray.map((oldConditions) => {
                        if (oldConditions.key !== condition.key) return oldConditions
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                    ...
                    const updatedConditions = collectionInfo.conditions.conditionArray.map((oldCondition) => {
                      if (oldCondition.key !== condition.key) return oldCondition
              ...
              <Box>
                <Button
                  ...
                  onClick={() => {
                    setCollectionInfo((prevState: CollectionInfo): CollectionInfo => {
                      const updatedConditions = collectionInfo.conditions.conditionArray.filter((oldCondition) => {
