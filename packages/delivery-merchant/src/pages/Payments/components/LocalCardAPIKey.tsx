import { Button, Card, Center, Checkbox, Group, Paper, Skeleton, Stack, Text, TextInput } from '@mantine/core'
...
const LocalCardAPIKey: React.FC<LocalCardAPIKeyProps> = ({
  ...
  return (
    ...
    >
      <Group 
      position='right'
      ><Checkbox label={t('Include local card fee in price')} checked={includeLocalCardFeeToPrice} onClick={() => {
        updateLocalCardSetting()
      }}/></Group>
      ...
      {apiKeysFilled ? (
        ...
        >
          ...
          // direction="column"
          >
            <TextInput {...form.getInputProps('MerchantId')} label={t('Merchant Id')} />
            <TextInput label={t('Terminal Id')} {...form.getInputProps('TerminalId')} />
            <TextInput label={t('Secret key')} {...form.getInputProps('secretKey')} />
          </Stack>
          <Center mt={22}>
            <Group>
              ...
              {localCardAPIKeyFilled && <Button onClick={() => setApiKeysFilled(true)}>{t('Cancel')}</Button>}
            </Group>
          </Center>
        </form>
      )}
