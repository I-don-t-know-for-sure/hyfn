import { Badge, Box, Button, Center, Group, Image, Modal, Stack, Table, Text } from '@mantine/core'
import { storeServiceFee } from 'config/constants'
...
const TransactionModal: React.FC<TransactionModalProps> = ({ transactions, validateTransaction, storeProducts }) => {
  const [opened, setOpened] = useState(false)
  const { userDocument } = useUser()
  const orderTotal = storeProducts.reduce((accu, currentProduct) => {
    return accu + currentProduct.pricing.price * currentProduct?.pickup?.QTYFound
  }, 0)
      ...
      <Modal opened={opened} onClose={() => setOpened(false)}>
        ...
        <Stack m={'sm'}>
          ...
          <Group position="apart">
            ...
            <Text>{`${userDocument.currency || 'LYD'} ${orderTotalAfterFee}`}</Text>
          </Group>
        </Stack>
