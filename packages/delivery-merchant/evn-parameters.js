...
;(function () {
  ...
  const cmd = cp.spawn(`aws ssm get-parameters-by-path --with-decryption --path /hyfn/frontend/${env}`, { shell: true })

  //   const cmd = cp.spawn(
  //     `aws ssm get-parameters-by-path --with-decryption --path /hyfn/delivery-merchant/${buildInfo.envName}`,
  //     { shell: true },
  //   )
  // echo output of the commands to the console
  const ready = []
  var result = buildInfo.envName === 'dev' ? 'GENERATE_SOURCEMAP=false' : 'GENERATE_SOURCEMAP=false'
...
const after = async ({ cp, env, fs }) => {
  const cmd2 = cp.spawn(
    `aws ssm get-parameters-by-path --with-decryption --path /hyfn/frontend/${env}/delivery-merchant`,
    { shell: true },
  )
