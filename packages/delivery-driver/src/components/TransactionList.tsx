import { Box, Button, Center, Container, Group, Loader, Modal, Paper, Text, UnstyledButton } from '@mantine/core'
...
const TransactionList: React.FC<TransactionListProps> = ({ menu }) => {
  ...
  const { data: transactions, isLoading, fetchNextPage } = useGetTransactions({ enabled: opened })
              ...
              return (
                <Paper shadow={'md'} withBorder key={transaction._id} mt={12}>
                  <Box m={'md'}>
                    ...
                    >
                      <>{TextWithLabel({ label: 'Store', value: transaction.storeId })}</>

                      <TextWithLabel label="Amount" value={transaction.amount || 0} />
                      <TextWithLabel label="Validated" value={`${transaction.validated}`} />
                    </Group>
                    {!transaction.validate && (
                      <Button onClick={() => validateTransaction({ transactionId: transaction._id })} mt={16} fullWidth>
                        {t('Validate')}
                      </Button>
                    )}
                  </Box>
              ...
              fetchNextPage({
                pageParam:
                  transactions?.pages[transactions?.pages?.length - 1][
                    transactions?.pages[transactions.pages?.length - 1]?.length - 1
                  ]?._id,
              })
            }
          >
