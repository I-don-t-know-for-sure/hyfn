interface CreatePaymentRequestObjectProps extends Omit<MainFunctionProps, "arg"> {
    arg: any;
}
import { KMS } from "aws-sdk";
import { createLocalCardConfigurationObject, decryptData, MainFunctionProps, mainWrapper, } from "hyfn-server";
const { ObjectId } = require("mongodb");
export const createStoreLocalCardTransaction = async ({ arg, client, session, }: MainFunctionProps) => {
    ...
    //         },
    //       },
    //       options: { arrayFilters: [{ 'store._id': new ObjectId(paymentRequestId) }] },
